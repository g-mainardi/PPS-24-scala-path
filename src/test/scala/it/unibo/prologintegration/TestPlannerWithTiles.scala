...
class TestPlannerWithTiles extends AnyFlatSpec with Matchers with TestPlanner {
  ...
  "PlannerWithTiles" should "find a valid path with max moves" in :
    val plan = PlannerWithTiles((0, 0), (2, 2), passableTiles, Some(7)).plan
    plan shouldBe a[SucceededPlan]

  "PlannerWithTiles" should "find a valid path with directions" in :
    val plan = PlannerWithTiles((0, 0), (2, 2), passableTiles, Some(7), allDirections).plan
    plan shouldBe a[SucceededPlan]
